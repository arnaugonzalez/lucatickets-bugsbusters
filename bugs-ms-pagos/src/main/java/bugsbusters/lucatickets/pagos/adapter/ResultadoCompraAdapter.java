...
/**
 * ResultadoCompraAdapter Microservicio pagos 18/03/2024 V1 BugsBusters
 */
@Component
public class ResultadoCompraAdapter {
	...
	public ResultadoCompraResponse crearResultadoCompra(ResultadoPagoResponse resultadoPagoResponse, Compra compra) {
		...
		resultado.setCodigo(
				resultadoPagoResponse.getError().substring(0, Math.min(resultadoPagoResponse.getError().length(), 8)));

//		List<String> mensajes = new ArrayList<String>();
//		int i = 0;
//		for(String m: resultadoPagoResponse.getMessage())
//			mensajes.add(m);
		resultado.setMensaje(resultadoPagoResponse.getError().substring(10, resultadoPagoResponse.getError().length()));
